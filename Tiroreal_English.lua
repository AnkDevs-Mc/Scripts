...
-- store ESP
local mobESP = {}
local MAX_DISTANCE = 5000 -- 👈 only scan mobs within a range of 5000 studs

-- create circle
local function createCircle()
    local circle = Drawing.new("Circle")
    circle.Color = Color3.fromRGB(0, 255, 0) -- green
    circle.Thickness = 2
    circle.NumSides = 50
    circle.Filled = false
    circle.Radius = 1.2 -- 👈 10 times smaller (compared to 12)
    circle.Visible = true
    return circle
end
...
-- update the circle according to position
...
task.delay(25, function() -- ⏳ Wait 10 seconds before enabling this script

    -- function to create aqua aura
    local function createAquaAura(char)
        ...
        aura.Name = "AquaAura"
        aura.FillColor = Color3.fromRGB(64, 224, 208)   -- aqua
        aura.OutlineColor = Color3.fromRGB(64, 224, 208)
        aura.FillTransparency = 1 -- hidden by default
        aura.OutlineTransparency = 1
        aura.Parent = char
    end

    -- create aura on respawn
...
Notification.new("<Color=Blue>Happy 2026 And Goodbye 2025<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Happy Tet Everyone<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Long Live Ho Chi Minh<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Long Live Great Uncle Ho<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Happy New Year to Vietnam<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Fuck Cali<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Brain-dead Diem eats shit<Color=/>"):Display()
task.wait(3)
Notification.new("<Color=Blue>Tiro Hub loves Uncle Ho<Color=/>"):Display()
task.wait(3)
...
-- Customization
local TEXT = "Tiro Hub"
local TEXT_SIZE = 14                 -- text size (not too large)
local GUI_OFFSET = Vector3.new(0, 1.8, 0) -- offset from the head
local RAINBOW_SPEED = 1.0           -- color change speed (1 = normal, increase to make faster)
...
local function createBillboardFor(character)
    ...
    -- If there is already a Billboard created by the script, destroy it first
    ...
    billboard.Size = UDim2.new(0, 200, 0, 40) -- GUI size
...
local function onCharacterAdded(character)
    -- wait for the head to appear
    if not character.Parent then
        character.AncestryChanged:Wait()
    end
    -- create after 0.1s to make sure the head exists
    wait(0.1)
...
-- optional: if you want to disable when leaving the game (cleanup)
player.AncestryChanged:Connect(function(_, parent)
    if not parent then
        -- client is being removed, nothing to do
    end
end)
...
textLabel.Size = UDim2.new(0, 200, 0, 40) -- Reduce the box size
textLabel.Position = UDim2.new(0, 10, 0, 10)
textLabel.Font = Enum.Font.FredokaOne
textLabel.TextScaled = false
textLabel.TextSize = 20 -- Small text size
textLabel.BackgroundTransparency = 1
            ...
            if _G.ChestESP then
                if not chest:GetAttribute("IsDisabled") then
                    if not chest:FindFirstChild("ChestEsp") then
                        ...
                        name.BackgroundTransparency = 1
                        name.TextStrokeTransparency = 0.5
                        name.TextColor3 = Color3.fromRGB(255, 215, 0) -- Yellow color for chest
                    else
...
ImageButton.BackgroundTransparency = 1 -- Transparent background
ImageButton.BorderSizePixel = 0
ImageButton.Position = UDim2.new(0.120833337 - 0.10, 0, 0.0952890813 + 0.01, 0)
ImageButton.Size = UDim2.new(0, 50, 0, 50)
ImageButton.Draggable = true
ImageButton.Image = "rbxassetid://79932823311771"
	...
	coroutine.resume(
		coroutine.create(
			function()
				...
				Circle.ZIndex = 10
				Circle.Image = "rbxassetid://79932823311771"
...
Discord.BackgroundTransparency = 1 -- Make the background transparent
...
Disc_Logo.Image = "http://www.roblox.com/asset/?id=79932823311771"
...
-- Click event: copy TikTok link
Discord.MouseButton1Click:Connect(function()
    (setclipboard or toclipboard)("tiktok.com/@tirohub")
    wait(.1)
...
UICorner.Parent = ImageButton
	...
	function tabs:Tab(Name, icon)
		...
		ImageLabel.Image = "http://www.roblox.com/asset/?id=79932823311771" .. icon
	   ...
	   Sep2.TextXAlignment = Enum.TextXAlignment.Center
		...
		function functionitem:Button(Name, callback)
			...
			function functionitem:Toggle(Name, default, callback)
				...
				MainToggle_3.Image = "http://www.roblox.com/asset/?id=79932823311771"
			...
			function functionitem:Dropdown(Name, list, default, callback)
				...
				ImageButton.Image = "http://www.roblox.com/asset/?id=79932823311771"
  ...
  function functionitem:Slider(text, floor, min, max, de, callback)
    ...
    -- Calculate the value while dragging the slider
    ...
    -- Update while dragging the slider
    ...
    -- Update the value when CustomValue changes
    ...
    while wait() do
        if _G.CollectBerry then
            ...
            if NearestBush and NearestBerryName then
                ...
                -- Find the object that is actually the berry (by attribute name)
                local BerryPart = BushModel:FindFirstChild(NearestBerryName)
                if BerryPart and BerryPart:IsA("BasePart") then
                    -- Fly to the exact berry position
                    TP1(BerryPart.CFrame + Vector3.new(0, 1, 0)) -- hover above the berry a little
                    task.wait(0.3)

                    -- Press E to pick up the berry
    ...
    while wait() do
     ...
     FM = Trailers:Label("Full Moon")
        ...
        task.spawn(function()
            while task.wait() do
                pcall(function()
                    if game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=79932823311771" then
                        FM:Set("Full Moon: 100")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=79932823311771" then
                        FM:Set("Full Moon: 75")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=79932823311771" then
                        FM:Set("Full Moon: 50")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=79932823311771" then
...
-- Disable/enable rendering when the window is focused
...
-- Create data to send to Discord
local Data = {
    ["embeds"] = {
        {
            ...
            ["color"] = tonumber("0x000000"), -- Change the outline color to black
...
game:GetService("StarterGui"):SetCore(
    "SendNotification",
    {
        ...
        Icon = "rbxassetid://79932823311771",
        Duration = 5
    }
)
-- Change all current skills + newly appearing skills to rainbow colors
...
-- Change all existing ones
for _, obj in ipairs(workspace:GetDescendants()) do
    rainbowSkill(obj)
end

-- Listen for new skills
